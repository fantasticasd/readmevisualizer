...
function normalizeAst(ast: Root): { tokens: Token[]; links: Array<{ href: string; text: string }> } {
    ...
    // inside pargaraphs, list items, blockquotes, etc.)
    ...
    visit(ast, 'text', (node: Text) => {
        ...
        while ((match = rawUrlRegex.exec(value)) !== null) {
            links.push({
                href: match[0],
                text: match[0],
            })
    ...
    return { tokens, links: dedupedLinks }
}
 ...
 * 
 * Content nodes are assignes to the section that immediately precedes them 
 * in source order(0() single pass, no index lookups).
 */
function buildSectionTree(
    tokens: Token[],
    dedupMap: Map<string,number>,
): { sections: DocumentSection[]; flatSections: DocumentSection[] } {
    // --- 1. Collect headinf tokens in source order ---
    ...
    for (const section of allSections) {
        // Pop ancestors that oare at the same or deeper level
     while (
...
function computeStats(
    ...
    for (const section of flatSections) {
        // Skip the synthetics fallback title - it is not derived from source text
...
// Results are reused across React re-renders witout re-parsing.
// ---------------------------------------------------------------------------
